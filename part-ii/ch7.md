...
[图7-1](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-1.png)是这类问题的一个简单例子。假设你有两个客户端同时在数据库中增长一个计数器。（假设数据库中没有自增操作）每个客户端需要读取计数器的当前值，加 1 ，再回写新值。[图7-1](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-1.png) 中，因为发生了两次增长，计数器应该从42增至44；但由于竞态条件，实际上只增至 43 。
...
这些定义假设你想同时修改多个对象（行，文档，记录）。通常需要**多对象事务（multi-object transaction）** 来保持多块数据同步。[图7-2](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-2.png)展示了一个来自电邮应用的例子。执行以下查询来显示用户未读邮件数量：
...
在[图7-2](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-2.png)中，用户2 遇到异常情况：邮件列表里显示有未读消息，但计数器显示为零未读消息，因为计数器增长还没有发生。隔离性可以避免这个问题：通过确保用户2 要么同时看到新邮件和增长后的计数器，要么都看不到。反正不会看到执行到一半的中间结果。

![](../.gitbook/assets/fig7-2.png)

**图7-2 违反隔离性：一个事务读取另一个事务的未被执行的写入（“脏读”）。**

[图7-3](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-3.png)说明了对原子性的需求：如果在事务过程中发生错误，邮箱和未读计数器的内容可能会失去同步。在原子事务中，如果对计数器的更新失败，事务将被中止，并且插入的电子邮件将被回滚。
...
一些数据库也提供更复杂的原子操作，例如自增操作，这样就不再需要像 [图7-1](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-1.png) 那样的读取-修改-写入序列了。同样流行的是 [**比较和设置（CAS, compare-and-set）**](ch7.md#比较并设置（CAS）) 操作，当值没有被其他并发修改过时，才允许执行写操作。
...
* 在文档数据模型中，需要一起更新的字段通常在同一个文档中，这被视为单个对象——更新单个文档时不需要多对象事务。但是，缺乏连接功能的文档数据库会鼓励非规范化（参阅“[关系型数据库与文档数据库在今日的对比](../part-i/ch2.md#关系型数据库与文档数据库在今日的对比)”）。当需要更新非规范化的信息时，如 [图7-2](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-2.png) 所示，需要一次更新多个文档。事务在这种情况下非常有用，可以防止非规范化的数据不同步。
...
* 如果事务需要更新多个对象，脏读取意味着另一个事务可能会只看到一部分更新。例如，在[图7-2](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-2.png)中，用户看到新的未读电子邮件，但看不到更新的计数器。这就是电子邮件的脏读。看到处于部分更新状态的数据库会让用户感到困惑，并可能导致其他事务做出错误的决定。
* 如果事务中止，则所有写入操作都需要回滚（如[图7-3](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-3.png)所示）。如果数据库允许脏读，那就意味着一个事务可能会看到稍后需要回滚的数据，即从未实际提交给数据库的数据。想想后果就让人头大。
...
* 如果事务更新多个对象，脏写会导致不好的结果。例如，考虑 [图7-5](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-5.png)，[图7-5](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-5.png) 以一个二手车销售网站为例，Alice和Bob两个人同时试图购买同一辆车。购买汽车需要两次数据库写入：网站上的商品列表需要更新，以反映买家的购买，销售发票需要发送给买家。在[图7-5](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-5.png)的情况下，销售是属于Bob的（因为他成功更新了商品列表），但发票却寄送给了爱丽丝（因为她成功更新了发票表）。读已提交会阻止这样这样的事故。
...
但是在使用此隔离级别时，仍然有很多地方可能会产生并发错误。例如[图7-6](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-6.png)说明了读已提交时可能发生的问题。
...
现在想象一下，Alice和Bob是两位值班医生。两人都感到不适，所以他们都决定请假。不幸的是，他们恰好在同一时间点击按钮下班。[图7-8](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-8.png)说明了接下来的事情。
...
​ 出于这个原因，具有单线程串行事务处理的系统不允许交互式的多语句事务。取而代之，应用程序必须提前将整个事务代码作为存储过程提交给数据库。这些方法之间的差异如[图7-9](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-9.png) 所示。如果事务所需的所有数据都在内存中，则存储过程可以非常快地执行，而不用等待任何网络或磁盘I/O。
...
之前我们看到锁通常用于防止脏写（参阅“[没有脏写](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/没有脏写/README.md)”一节）：如果两个事务同时尝试写入同一个对象，则锁可确保第二个写入必须等到第一个写入完成事务（中止或提交），然后才能继续。
...
第二种情况要考虑的是另一个事务在读取数据之后修改数据。这种情况如[图7-11](https://github.com/tiandiyijian/ddia/tree/03cdebe14167136bb3811de33d5a5e603260003f/img/fig7-11.png)所示。
